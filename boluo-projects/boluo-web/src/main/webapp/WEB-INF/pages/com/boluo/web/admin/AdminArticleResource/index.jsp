...
<div class="container row-space-top-2">
  ...
  <div class="row row-space-top-4">
    <c:forEach var="item" items="${articles}">
      <div class="col-md-3 col-sm-4 col-xs-6" id="article_${item.id}">
        <div class="panel panel-default" data-description="${item.description}" data-id="${item.id}" data-image="${item.image}"
              ...
              <div class="cover">
                <c:choose>
                  <c:when test="${not empty item.image}">
                    <a href="/article/${item.id}" target="_blank"><img src="http://wfenxiang.b0.upaiyun.com/${item.image}!M"></a>
                  </c:when>
                  <c:otherwise>
                    <a href="/article/${item.id}" target="_blank"><img data-src="holder.js/160x80"></a>
                  </c:otherwise>
                </c:choose>
              </div>
              <div class="title">
                <a href="/article/${item.id}" target="_blank" title="查看资讯">${item.title}</a><a href="${item.link}" target="_blank" title="查看原文"><span class="glyphicon glyphicon-link"
          ...
          <div class="panel-footer">
            ...
            <div class="pull-right">
              <a data-toggle="modal" data-target="#EditModal" href="#" title="编辑"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
              <!--  -->
  ...
  <div class="row">
    <div class="col-md-12">
      <ul class="pagination">
        <li><a href="/admin/article?page=1">首页</a></li>
        <c:forEach var="page" items="${requestScope.pages}">
          <c:choose>
            ...
            <c:otherwise>
              <li><a href="/admin/article?page=${page}">${page}</a></li>
            </c:otherwise>
          </c:choose>
        </c:forEach>
        <li><a href="/admin/article?page=${lastPage}">尾页</a></li>
      </ul>
    </div>
  </div>
</div>
...
<div class="modal" id="EditModal" tabindex="-1" role="dialog" aria-labelledby="EditModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="/admin/article/update" class="form-horizontal" id="edit-form" method="post" name="edit-form" role="form">
        ...
        <div class="modal-footer">
          <input id="edit-articleId" name="articleId" type="hidden"><input id="edit-image" name="image" type="hidden">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="/admin/article/add_topic" id="topic-form" method="post" name="topic-form" role="form">
          <div class="form-group">
            <label for="topic-title">加入专题</label><input class="form-control" id="topic-title" name="title">
          </div>
          <div class="form-group">
            <input id="topic-articleId" name="articleId" type="hidden">
            <button type="submit" class="btn btn-success">提交</button>
          </div>
        </form>
      </div>
...
<script src="/js/admin/article.js?v=20160816"></script>